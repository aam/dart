...
main() {
  ...
  server.listen("127.0.0.1", 8180); // and start listening
  print("listening on http://127.0.0.1:8180");
}
...
void myAppHandler(HttpRequest req, HttpResponse res) {
  print ("${req.path}");
  var s = req.path;
  if (s.startsWith("/MyApp"))
    s = s.substring("/MyApp".length);
  print(s);
  File file = new File(".${s}"); //open the file, eg: ./MyApp.html
